...
contract TransactionRegistry is ERC1155, AccessControl {
    ...
    // Mapping from transaction hash to token ID (for lookup)
    ...
    constructor() ERC1155("https://api.kyc-kyb-platform.io/transaction/{id}.json") {
    ...
    function registerTransaction(
        ...
        emit TransactionRegistered(tokenId, msg.sender, txHash, txType, block.timestamp);
    ...
    function batchRegisterTransactions(
        ...
        require(
            txHashes.length == txTypes.length && txTypes.length == metadatas.length,
        ...
        for (uint256 i = 0; i < txHashes.length; i++) {
            ...
            require(txHashToToken[txHashes[i]] == 0, "Transaction already registered");
     ...
     */
    function verifyTransaction(bytes32 txHash) external view returns (
        bool exists,
        uint256 tokenId,
        bool isValid
    ) {
     ...
     */
    function getTransactionDetails(uint256 tokenId) external view returns (
        bytes32 txHash,
        string memory txType,
        bytes32 metadata,
        ...
        require(txData.registeredBy != address(0), "Transaction does not exist");
     ...
     */
    function invalidateTransaction(uint256 tokenId) external onlyRole(REGISTRAR_ROLE) {
